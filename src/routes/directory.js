...
const PUBLIC_COLS = `id, name, email, emp_code, role, department, title, phone, shift_start, join_date, exit_date, birthday, asana_email, created_ts, active`;
...
const insertUser = db.prepare(
  `INSERT INTO users (name, email, password_hash, emp_code, role, department, title, phone, shift_start, join_date, exit_date, birthday, asana_email, leave_balance, active, created_ts)
   VALUES (@name, @email, @password_hash, @emp_code, @role, @department, @title, @phone, @shift_start, @join_date, @exit_date, @birthday, @asana_email, @leave_balance, 1, @created_ts)`
);
const updateUser = db.prepare(
  `UPDATE users SET name=@name, email=@email, emp_code=@emp_code, role=@role, department=@department,
   title=@title, phone=@phone, shift_start=@shift_start, join_date=@join_date, exit_date=@exit_date, birthday=@birthday, asana_email=@asana_email, leave_balance=@leave_balance WHERE id=@id`
);
...
router.post('/', requireAdmin, (req, res) => {
  ...
  const info = insertUser.run({
    ...
    asana_email: String(req.body.asana_email || '').trim().toLowerCase().slice(0,160),
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  updateUser.run({
    ...
    asana_email: String(req.body.asana_email ?? existing.asana_email ?? '').trim().toLowerCase().slice(0,160),
